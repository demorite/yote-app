...
class MatchDetailsScreen extends React.Component {
	...
	render() {
		...
		const imageUrl = 'https://picsum.photos/600?random&' + encodeURI(match.place);

		const playerItem = (p, i) => {
			const toUserDetails = () => this.props.navigation.navigate('UserDetails', {user: p.id});
			return <TouchableOpacity key={i}
			                         onPress={toUserDetails}>
		...
		return <ScrollView>
			<Card
				...
				<TouchableOpacity onPress={() => {
					if (match.babyfoot)
						this.props.navigation.navigate('BabyFootDetails', {babyFoot: match.babyfoot.id})
				}}>
					<Text>
						<Bold>Lieu : </Bold>
						<Text>{`${match.babyfoot ? match.babyfoot.place : 'Pas de baby-foot'}`}</Text>
					</Text>
				</TouchableOpacity>
				<HorizontalDivider>
				...
				<Text>
					<Bold>Durée : </Bold>
					<Text>{`${match.duration}`}</Text></Text>
				<Text>
					<Bold>Joué : </Bold>
					<Text>{`${match.played ? 'Oui' : 'Non'}`}</Text></Text>
				<Text>
					<Bold>En cours : </Bold>
					<Text>{`${match.inUse ? 'Oui' : 'Non'}`}</Text></Text>
				<Text>
					<Bold>Score (Rouge - Bleu) : </Bold>
					<Text>{`${match.redScore} - ${match.blueScore}`}</Text></Text>
				<Text>
					<Bold>Score (Rouge - Bleu) : </Bold>
					<Text>{`${match.redScore} - ${match.blueScore}`}</Text></Text>
			</Card>
